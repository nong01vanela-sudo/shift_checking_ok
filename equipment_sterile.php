...
$shiftConfig = [
    'day' => [
        'name'        => 'เวรเช้า',
        'table'       => 'day_check_equipment_sterile',
        'prefix'      => 'd',
        'prev_name'   => 'เวรดึก (เวรก่อนหน้า)',
        'prev_table'  => 'night_check_equipment_sterile',
        'prev_prefix' => 'n'
    ],
    'eve' => [
        'name'        => 'เวรบ่าย',
        'table'       => 'eve_check_equipment_sterile',
        'prefix'      => 'e',
        'prev_name'   => 'เวรเช้า (เวรก่อนหน้า)',
        'prev_table'  => 'day_check_equipment_sterile',
        'prev_prefix' => 'd'
    ],
    'night' => [
        'name'        => 'เวรดึก',
        'table'       => 'night_check_equipment_sterile',
        'prefix'      => 'n',
        'prev_name'   => 'เวรบ่าย (เวรก่อนหน้า)',
        'prev_table'  => 'eve_check_equipment_sterile',
        'prev_prefix' => 'e'
    ]
];
...
// Fetch catalog
$catalogStmt = $pdo->query("SELECT * FROM `stock_equipment_sterile` ORDER BY `id` ASC");
...
try {
    $prevQuery = "
        ...
        INNER JOIN (
            SELECT `equipment_sterile_name`, MAX(`id`) as max_id
            FROM `{$prevTable}`
            GROUP BY `equipment_sterile_name`
        ) t2 ON t1.`id` = t2.max_id
    ";
    foreach ($pdo->query($prevQuery)->fetchAll() as $row) {
        $prevData[$row['equipment_sterile_name']] = [
...
try {
    ...
    foreach ($stmt->fetchAll() as $row) {
        $currentData[$row['equipment_sterile_name']] = [
            'id'       => (int)$row['id'],
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'save_stock') {
    ...
    try {
        $insertSql  = "INSERT INTO `{$currentTable}` (`equipment_sterile_name`,`{$p}_balance`,`{$p}_supply`,`{$p}_borrow`,`{$p}_borrower`,`{$p}_checker`) VALUES (?,?,?,?,?,?)";
        ...
        foreach ($catalog as $item) {
            $name = $item['equipment_sterile_name'];
        ...
        header('Location: equipment_sterile.php');
        exit;
...
<head>
    ...
    <title>เครื่องมือ sterile | Stock Checking System</title>
...
<div class="app-container">
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <div class="sidebar-top">
            ...
            <nav class="sidebar-menu">
                ...
                <a href="equipment_sterile.php" class="nav-link active" title="เครื่องมือ sterile">
    ...
    <main class="main-content">
        <header class="page-header">
            <div>
                <h1 class="page-title">เครื่องมือ sterile</h1>
        ...
        <form method="POST" action="equipment_sterile.php" id="stockForm">
                ...
                <table class="flat-table">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th rowspan="2" class="col-name">ชื่อเครื่องมือ Sterile</th>
...
const DRAFT_KEY = 'equipment_sterile_draft_' + USER_ID + '_' + CURRENT_SHIFT;
