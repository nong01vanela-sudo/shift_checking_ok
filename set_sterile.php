...
$shiftConfig = [
    'day' => [
        'name'        => 'เวรเช้า',
        'table'       => 'day_check_set_sterile',
        'prefix'      => 'd',
        'prev_name'   => 'เวรดึก (เวรก่อนหน้า)',
        'prev_table'  => 'night_check_set_sterile',
        'prev_prefix' => 'n'
    ],
    'eve' => [
        'name'        => 'เวรบ่าย',
        'table'       => 'eve_check_set_sterile',
        'prefix'      => 'e',
        'prev_name'   => 'เวรเช้า (เวรก่อนหน้า)',
        'prev_table'  => 'day_check_set_sterile',
        'prev_prefix' => 'd'
    ],
    'night' => [
        'name'        => 'เวรดึก',
        'table'       => 'night_check_set_sterile',
        'prefix'      => 'n',
        'prev_name'   => 'เวรบ่าย (เวรก่อนหน้า)',
        'prev_table'  => 'eve_check_set_sterile',
        'prev_prefix' => 'e'
    ]
];
...
// Fetch catalog
$catalogStmt = $pdo->query("SELECT * FROM `stock_set_sterile` ORDER BY `id` ASC");
...
try {
    $prevQuery = "
        ...
        INNER JOIN (
            SELECT `set_name`, MAX(`id`) as max_id
            FROM `{$prevTable}`
            GROUP BY `set_name`
        ) t2 ON t1.`id` = t2.max_id
    ";
    foreach ($pdo->query($prevQuery)->fetchAll() as $row) {
        $prevData[$row['set_name']] = [
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'save_stock') {
    ...
    try {
        $insertSql  = "INSERT INTO `{$currentTable}` (`set_name`,`{$p}_balance`,`{$p}_supply`,`{$p}_borrow`,`{$p}_borrower`,`{$p}_checker`) VALUES (?,?,?,?,?,?)";
        ...
        header('Location: set_sterile.php');
        exit;
...
<head>
    ...
    <title>SET sterile | Stock Checking System</title>
...
<div class="app-container">
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <div class="sidebar-top">
            ...
            <nav class="sidebar-menu">
                ...
                <a href="set_sterile.php" class="nav-link active" title="SET sterile">
    ...
    <main class="main-content">
        <header class="page-header">
            <div>
                <h1 class="page-title">SET sterile</h1>
        ...
        <form method="POST" action="set_sterile.php" id="stockForm">
                ...
                <table class="flat-table">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th rowspan="2" class="col-name">ชื่อเซต</th>
...
const DRAFT_KEY = 'set_sterile_draft_' + USER_ID + '_' + CURRENT_SHIFT;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (form) {
        ...
        form.addEventListener('change', handleFormInput); // To capture select changes
